...
// Load .env, then .env.local if present so developers can keep local secrets in .env.local
dotenv.config();
try {
  // dotenv will return an object but we don't need it; just attempt to load .env.local if it exists
  dotenv.config({ path: '.env.local' });
  console.log('Loaded .env and .env.local (if present)');
} catch (e) {
  console.warn("Could not load .env.local (this is fine if you don't have one)");
}
...
app.post('/api/enquiry', async (req, res) => {
  ...
  // include branch in subject so owner immediately sees which branch the enquiry is for
  const subject = `New seat enquiry: ${branch} - ${firstName} ${lastName}`;
  const text = `New enquiry from ${firstName} ${lastName}\nPhone: ${phone}\nShift: ${shift}\nBranch: ${branch}\nPlan: ${duration}`;
  const html = `<p><strong>New enquiry</strong></p><ul><li><strong>Name:</strong> ${firstName} ${lastName}</li><li><strong>Phone:</strong> ${phone}</li><li><strong>Shift:</strong> ${shift}</li><li><strong>Branch:</strong> ${branch}</li><li><strong>Plan:</strong> ${duration}</li></ul>`;
