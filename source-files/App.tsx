...
import RateUsOverlay from './components/RateUsOverlay';
...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <RateUsOverlay />
      {/* Floating Dhruv Library logo in hero left - only on home/hero, inside hero section */}
      {route === 'home' && (
        <div className="relative w-full max-w-7xl mx-auto">
          <div className="absolute left-0 top-10 z-30 hidden md:block animate-float-logo">
            <img src="/img/logo rm.png" alt="Dhruv Library Logo" className="w-80 h-80 object-contain drop-shadow-2xl opacity-95" />
          </div>
        </div>
      )}
      <style>{`
        ...
        .animate-float-logo { animation: float-logo 3.5s ease-in-out infinite; }
      `}</style>
      <style>{`
        /* hero bubbles */
      ...
      <nav className={`fixed w-full z-50 transition-all duration-300 ${
        route === 'contact'
          ? 'bg-white/95 backdrop-blur-md shadow-sm border-b border-orange-100 py-2'
          : (scrolled ? 'bg-white/95 backdrop-blur-md shadow-sm border-b border-orange-100 py-2' : 'bg-transparent py-4')
      }`}>
  ...
  <section id="home" className="relative pt-32 pb-20 lg:pt-48 lg:pb-32 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          {/* SEO H1: exact keyword for local search (screen-reader-only so visual layout is unchanged) */}
          <h1 className="sr-only">Dhruv Library & Computer Center — Best Library in Meerut</h1>
  ...
  {route.startsWith('verify') && (
    // add top padding so verification page content clears the fixed site nav
    <div className="pt-20">
      <Suspense fallback={<Loader />}>
        <VerifyCertificate />
      </Suspense>
    ...
    <Suspense fallback={<Loader />}>
      ...
      <footer className="relative overflow-hidden bg-slate-900 text-slate-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                ...
                <a href="#" className="hover:text-amber-200">Admission</a>
                <a href="#" className="hover:text-amber-200">Courses</a>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-2 text-sm">
                <p>Have questions or want to join? Reach us at:</p>
                <a href="mailto:dhruvlibrary2022@gmail.com" className="block text-lg font-medium text-white hover:text-amber-200 transition">dhruvlibrary2022@gmail.com</a>
                <p className="text-slate-300 mt-4">Follow us on social media for updates and announcements:</p>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <a
                href="https://www.instagram.com/dhruv_library_meerut?igsh=d3J1bTBia3hmeW5r"
                className="hover:text-amber-200 group"
                target="_blank"
                rel="noopener noreferrer"
                onMouseEnter={() => {
                  const logo = document.getElementById('footer-logo-animated');
                  if (logo) logo.classList.add('insta-glow');
                }}
                onMouseLeave={() => {
                  const logo = document.getElementById('footer-logo-animated');
                  if (logo) logo.classList.remove('insta-glow');
                }}
              >
                <Instagram size={18} />
              </a>
              <a
                href="https://www.linkedin.com/company/dhruv-library-2022/"
                className="hover:text-amber-200 group"
                target="_blank"
                rel="noopener noreferrer"
                onMouseEnter={() => {
                  const logo = document.getElementById('footer-logo-animated');
                  if (logo) logo.classList.add('linkedin-glow');
                }}
                onMouseLeave={() => {
                  const logo = document.getElementById('footer-logo-animated');
                  if (logo) logo.classList.remove('linkedin-glow');
                }}
              >
                <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24"><path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-10h3v10zm-1.5-11.268c-.966 0-1.75-.784-1.75-1.75s.784-1.75 1.75-1.75 1.75.784 1.75 1.75-.784 1.75-1.75 1.75zm15.5 11.268h-3v-5.604c0-1.337-.025-3.063-1.868-3.063-1.868 0-2.154 1.459-2.154 2.968v5.699h-3v-10h2.881v1.367h.041c.401-.761 1.379-1.563 2.838-1.563 3.036 0 3.597 2.001 3.597 4.6v5.596z"/></svg>
              </a>
        ...
        <style>{`
          ...
          .footer-logo.insta-glow {
            animation: instaGlow 2.2s linear infinite;
            box-shadow: 0 0 32px 8px #fd1d1d, 0 0 64px 16px #fcb045, 0 0 24px 4px #833ab4;
          }
          .footer-logo.linkedin-glow {
            animation: linkedinGlow 2.2s linear infinite;
            box-shadow: 0 0 32px 8px #0077b5, 0 0 64px 16px #0a66c2, 0 0 24px 4px #005983;
          }
          ...
          @keyframes instaGlow {
            0%, 100% { box-shadow: 0 0 32px 8px #fd1d1d, 0 0 64px 16px #fcb045, 0 0 24px 4px #833ab4; }
            50% { box-shadow: 0 0 48px 16px #fd1d1d, 0 0 80px 32px #fcb045, 0 0 36px 8px #833ab4; }
          }
          @keyframes linkedinGlow {
            0%, 100% { box-shadow: 0 0 32px 8px #0077b5, 0 0 64px 16px #0a66c2, 0 0 24px 4px #005983; }
            50% { box-shadow: 0 0 48px 16px #0077b5, 0 0 80px 32px #0a66c2, 0 0 36px 8px #005983; }
          }
